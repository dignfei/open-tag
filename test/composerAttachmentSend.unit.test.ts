...
test("composer preserves the draft until the message request succeeds", () => {
  const src = fs.readFileSync(new URL("../web/src/views/Composer.tsx", import.meta.url), "utf8");
  const request = src.indexOf('const result = await api("POST", "/api/messages"');
  const responseError = src.indexOf("if (result?.error)");
  const clearText = src.indexOf('setText("")');
  const clearAttachments = src.indexOf("setPendingAtts([])");
  assert.ok(request >= 0 && responseError > request, "the parsed API error must be checked after the request");
  assert.ok(clearText > responseError && clearAttachments > responseError, "draft state must clear only after a successful response");
  assert.match(src, /catch \(e: any\)[\s\S]*toast\.error/, "network failures must surface without clearing the draft");
  assert.match(src, /sendingRef\.current/, "an in-flight send must reject duplicate submission");
});
