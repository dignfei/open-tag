...
// - read cursors are monotonic: a late request carrying an older seq cannot rewind a newer checkpoint.
//
...
async function main() {
  ...
  console.log("\n[5] a late stale read request cannot move the cursor backward");
  {
    ...
    await db.update(schema.channelMembers).set({ lastReadSeq: staleSeq }).where(and(eq(schema.channelMembers.channelId, channelId), eq(schema.channelMembers.memberType, "user"), eq(schema.channelMembers.memberId, viewerId)));
    check("fixture starts from the older cursor", (await readSeq(channelId)) === staleSeq);
