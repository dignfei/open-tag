...
for (const failure of [
  { name: "session error", event: { type: "session.error", data: { message: "provider rejected" } } },
  { name: "non-zero result", event: { type: "result", sessionId: "session-test", exitCode: 1 } },
]) test(`Copilot reports an admitted ${failure.name}`, () => {
  const source = `
const fs = require("node:fs");
const file = process.env.OPEN_TAG_TEST_STATE;
const count = (fs.existsSync(file) ? Number(fs.readFileSync(file, "utf8")) : 0) + 1;
fs.writeFileSync(file, String(count));
if (count === 2) console.log(${JSON.stringify(JSON.stringify(failure.event))});
`;
  return assertAcceptedFailureKeepsSessionReusable(adapters.find((adapter) => adapter.runtime === copilotRuntime)!, source);
});
test("Copilot keeps a session error authoritative over a zero result", () => {
  const source = `
const fs = require("node:fs");
const file = process.env.OPEN_TAG_TEST_STATE;
const count = (fs.existsSync(file) ? Number(fs.readFileSync(file, "utf8")) : 0) + 1;
...
test("Pi reports an admitted exit-zero model error", () => {
  const source = `
const fs = require("node:fs");
const file = process.env.OPEN_TAG_TEST_STATE;
const count = (fs.existsSync(file) ? Number(fs.readFileSync(file, "utf8")) : 0) + 1;
